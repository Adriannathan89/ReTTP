...
impl TemporaryFile {
    ...
    fn bytes(name: &str, content: &[u8]) -> Self {
        ...
        let directory =
            std::env::temp_dir().join(format!("rettp-cli-integration-{}-{id}", std::process::id()));
...
fn run(arguments: &[&str]) -> Output {
    rettp()
        .args(arguments)
        .output()
        .expect("rettp binary should run")
}
...
fn top_level_help_and_version_are_successful() {
    ...
    assert!(stdout(&help).contains("Usage: rettp <COMMAND>"));
...
fn bundled_basic_example_is_valid_and_reports_its_path() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/basic.rttp");
...
fn bundled_interpolation_example_accepts_repeated_var_options() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/interpolation.rttp");
...
fn lexical_diagnostic_has_phase_path_and_exact_location() {
    let source = TemporaryFile::text("lexical.rttp", "@");
    let output = run_check(source.path(), &[]);
...
fn syntax_diagnostics_have_phase_path_and_exact_location() {
    let source = TemporaryFile::text("syntax.rttp", r#"test "bad" {}"#);
    let output = run_check(source.path(), &[]);
...
fn missing_file_and_invalid_utf8_are_io_errors() {
    let missing_path = std::env::temp_dir().join(format!(
        "rettp-cli-definitely-missing-{}-{}.rttp",
        std::process::id(),
        NEXT_TEMPORARY_ID.fetch_add(1, Ordering::Relaxed)
    ));
    ...
    let invalid = TemporaryFile::bytes("invalid-utf8.rttp", &[0x66, 0x80, 0x6f]);
