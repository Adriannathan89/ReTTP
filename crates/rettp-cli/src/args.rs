...
#[derive(Parser)]
#[command(name = "rettp", version, about = "HTTP verification suite runner")]
pub(crate) struct Cli {
    /// Selected Rettp operation.
    #[command(subcommand)]
    pub(crate) command: Command,
}

/// Commands exposed by the `rettp` executable.
#[derive(Subcommand)]
pub(crate) enum Command {
    /// Checks a Rettp source file without executing HTTP requests.
    Check(CheckArgs),
    /// Checks and executes one Rettp source file.
    Run(RunArgs),
}
...
pub(crate) struct SourceArgs {
    /// Path to one UTF-8 `.rttp` source file.
...
/// Arguments accepted by `rettp check`.
#[derive(Args)]
...
/// Arguments accepted by `rettp run`.
#[derive(Args)]
