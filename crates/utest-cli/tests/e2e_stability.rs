...
impl TemporaryDirectory {
    fn new() -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("utest-stability-{}-{id}", std::process::id()));
...
fn complete_core_pipeline_capture_and_standalone_journey_passes() {
    ...
    let source = directory.write(
        "full_pipeline.utest",
        include_bytes!("fixtures/valid/full_pipeline.utest"),
    );
...
fn source_reader_enforces_utf8_and_the_five_mib_boundary_for_both_commands() {
    ...
    let exact_path = directory.write("exact.utest", exact);
    ...
    let oversized_path = directory.write("oversized.utest", vec![b' '; MAX_SOURCE_BYTES + 1]);
...
fn permanent_invalid_fixtures_stop_in_the_expected_checker_phase() {
    for (fixture, phase) in [
        ("fixtures/invalid/lexical.utest", "error[lexical]"),
        ("fixtures/invalid/syntax.utest", "error[syntax]"),
        ("fixtures/invalid/semantic.utest", "error[semantic]"),
    ] {
        let output = command()
            .arg("check")
            .arg(
