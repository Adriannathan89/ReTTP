...
impl TemporaryDirectory {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!("utest-cli-run-{}-{id}", std::process::id()));
...
fn run_requires_base_url_and_rejects_invalid_cli_or_configuration_with_four() {
    ...
    let source = directory.write("suite.utest", basic_source(200));
...
fn checker_runs_before_http_configuration_or_network() {
    let directory = TemporaryDirectory::new();
    let invalid = directory.write("invalid.utest", "@");
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
...
fn passed_failed_and_core_abort_use_exit_codes_zero_one_and_two() {
    ...
    let passed_source = directory.write("passed.utest", basic_source(200));
    ...
    let failed_source = directory.write("failed.utest", basic_source(201));
...
fn env_file_overrides_process_environment_and_cli_overrides_env_file() {
    ...
    let source = directory.write(
        "precedence.utest",
        r#"test "precedence" {
            request GET "/${UTEST_CLI_PRECEDENCE_VALUE}"
            expect { status = 200 }
        }"#,
    );
    let env_file = directory.write("values.env", "UTEST_CLI_PRECEDENCE_VALUE=env-file\n");
    ...
    let env_output = command()
        ...
        .arg("--env-file")
        .arg(env_path)
        .env("UTEST_CLI_PRECEDENCE_VALUE", "process")
        .output()
        .unwrap();
    ...
    let cli_output = command()
        ...
        .arg(env_path)
        .arg("--var")
        .arg("UTEST_CLI_PRECEDENCE_VALUE=first")
        .arg("--var")
        .arg("UTEST_CLI_PRECEDENCE_VALUE=cli")
        .env("UTEST_CLI_PRECEDENCE_VALUE", "process")
        .output()
        .unwrap();
...
fn timeout_is_a_normal_test_failure_and_output_failure_overrides_result_with_five() {
    let directory = TemporaryDirectory::new();
    let source = directory.write("timeout.utest", basic_source(200));
...
fn invalid_env_files_and_cli_assignments_are_code_four_and_value_free() {
    let directory = TemporaryDirectory::new();
    let source = directory.write("valid.utest", basic_source(200));
...
fn relative_report_path_uses_the_process_working_directory() {
    let directory = TemporaryDirectory::new();
    let source = directory.write("relative.utest", basic_source(200));
