...
use utest_domain::{BlockResult, ExecutionStatus, SuiteResult};
use utest_http::{HttpClientConfig, ReqwestHttpClient};
use utest_parser::{SourceText, ValidationContext};
use utest_reporter::{ColorMode, JsonReporter, JunitReporter, RunReport, TerminalReporter};
...
fn emit_diagnostics(
    source: &SourceText,
    diagnostics: &[utest_application::CheckDiagnostic],
) -> ExitCode {
