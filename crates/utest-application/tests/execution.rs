...
use utest_application::{ExecutionEngine, check_source};
use utest_domain::{
...
use utest_runtime::{RuntimeResolver, VariableStore, VariableValue};
...
fn test_result(result: &BlockResult) -> &utest_domain::TestResult {
...
fn request_values_are_resolved_before_transport() {
    ...
    let request = HttpRequestSpec::new(HttpMethod::POST, "/submit")
        ...
        .with_body(utest_domain::RequestBody::Text("payload ${TOKEN}".into()));
    let test = TestCase::new(
        "resolved",
        request,
        ResponseExpectation {
    ...
    assert!(
        matches!(&sent.body, Some(utest_http::ResolvedRequestBody::Text(body)) if body == "payload abc")
    );
}
