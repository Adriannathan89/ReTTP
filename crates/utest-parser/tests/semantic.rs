...
fn ast(input: &str) -> SuiteAst {
    let source = SourceText::new("semantic-test.utest", input);
    let lexed = lex(&source);
    assert!(lexed.is_success(), "lexer errors: {:?}", lexed.errors);
    parse(&lexed.tokens).ast
}

fn validate(input: &str) -> utest_parser::ValidationResult {
    validate_and_convert(&ast(input), &ValidationContext::new())
}

fn kinds(result: &utest_parser::ValidationResult) -> Vec<&ValidationErrorKind> {
    result.errors.iter().map(|error| &error.kind).collect()
}

fn has_kind(
    result: &utest_parser::ValidationResult,
...
fn reports_duplicate_blocks_declarations_sections_and_named_entries() {
    ...
    assert!(result.suite.is_none());
    use utest_parser::DuplicateKind as D;
    for duplicate in [
        D::CoreBlock,
        D::Request,
        D::Expectation,
...
fn validates_nested_object_rules_and_missing_field_assertions() {
    ...
    test.expectations[0].sections = vec![utest_parser::ExpectationSectionAst::Body(
...
fn suite_with_nested_header(depth: usize) -> SuiteAst {
    ...
    test.requests[0].sections = vec![utest_parser::RequestSectionAst::Headers(
        utest_parser::RequestHeadersAst {
            entries: vec![utest_parser::HeaderValueEntryAst {
...
fn validation_errors_expose_spans_display_and_error_traits() {
    ...
    let labels = [
        utest_parser::DuplicateKind::CoreBlock,
        utest_parser::DuplicateKind::Request,
        utest_parser::DuplicateKind::Expectation,
        utest_parser::DuplicateKind::RequestHeaders,
        utest_parser::DuplicateKind::RequestQuery,
        utest_parser::DuplicateKind::RequestBody,
        utest_parser::DuplicateKind::ResponseStatus,
        utest_parser::DuplicateKind::ResponseHeaders,
        utest_parser::DuplicateKind::ResponseBody,
        utest_parser::DuplicateKind::RequestHeader,
        utest_parser::DuplicateKind::ResponseHeader,
        utest_parser::DuplicateKind::QueryParameter,
        utest_parser::DuplicateKind::ObjectKey,
        utest_parser::DuplicateKind::AssertionField,
    ];
    for label in labels {
...
fn duplicate_core_is_validated_in_an_isolated_scope() {
    ...
    assert!(has_kind(&result, |kind| matches!(
        ...
        ValidationErrorKind::Duplicate {
            kind: utest_parser::DuplicateKind::CoreBlock
        }
    )));
    assert!(has_kind(&result, |kind| matches!(kind,
