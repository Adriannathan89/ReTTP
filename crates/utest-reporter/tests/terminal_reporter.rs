...
use utest_reporter::{ColorMode, RunReport, TerminalReporter};
...
fn renders_unnamed_empty_blocks_and_optional_failure_fields() {
    ...
    report.error = None;
    report.blocks.clear();
    report.blocks.push(utest_reporter::ReportBlock {
        kind: utest_reporter::ReportBlockKind::Core,
        name: None,
        status: utest_reporter::ReportStatus::Passed,
        duration_ms: 0,
        tests: vec![utest_reporter::ReportTest {
            name: "minimal".into(),
            status: utest_reporter::ReportStatus::Failed,
            duration_ms: 0,
            failures: vec![utest_reporter::ReportAssertionFailure {
                path: "body".into(),
                kind: utest_reporter::ReportFailureKind::MissingField,
                expected: None,
                actual: None,
    ...
    report.blocks.push(utest_reporter::ReportBlock {
        kind: utest_reporter::ReportBlockKind::Pipeline,
        name: None,
        status: utest_reporter::ReportStatus::Passed,
        duration_ms: 0,
        tests: Vec::new(),
...
fn plain_mode_escapes_controls_from_every_free_form_report_field() {
    ...
    test.error = Some(utest_reporter::ReportExecutionError {
        kind: utest_reporter::ReportErrorKind::Internal,
        message: "test-error\u{1b}[31m".into(),
    });
    report.error = Some(utest_reporter::ReportExecutionError {
        kind: utest_reporter::ReportErrorKind::Internal,
