use rettp_parser::{LexerErrorKind, SourceLocation, SourceSpan, SourceText, TokenKind, lex};

fn lex_ok(input: &str) -> Vec<TokenKind> {
    let source = SourceText::new("test.rttp", input);
...
fn reports_float_overflow_without_stopping() {
    let source = SourceText::new("numbers.rttp", format!("{}.0 core", "9".repeat(400)));
    let result = lex(&source);
    assert!(matches!(
...
fn lexes_a_representative_pipeline_source() {
    ...
    "#;
    let source = SourceText::new("pipeline.rttp", input);
    let result = lex(&source);
...
fn records_byte_spans_and_eof_for_unicode_source() {
    let source = SourceText::new("unicode.rttp", "test \"你好\"");
    let result = lex(&source);
...
fn source_text_slice_and_location_are_utf8_boundary_safe() {
    let source = SourceText::new("unicode.rttp", "你a\nxy");
    assert_eq!(source.name(), "unicode.rttp");
...
fn empty_source_text_reports_empty_and_first_location() {
    let source = SourceText::new("empty.rttp", "");
    assert!(source.is_empty());
...
fn collects_unexpected_characters_and_keeps_eof() {
    let source = SourceText::new("invalid.rttp", "@ ? $ - /");
    let result = lex(&source);
...
fn recovers_from_invalid_escape_at_newline() {
    let source = SourceText::new("invalid-string.rttp", "\"bad\\q\ncore");
    let result = lex(&source);
    assert!(matches!(
        result.errors.as_slice(),
...
fn reports_unterminated_string_at_eof() {
    let source = SourceText::new("invalid-string.rttp", "\"unfinished");
    let result = lex(&source);
    assert!(
