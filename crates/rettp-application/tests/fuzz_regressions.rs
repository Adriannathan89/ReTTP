...
use rettp_parser::{SourceSpan, SourceText, TokenKind, ValidationContext, lex, parse};

const FRONTEND_RECOVERY: &str = include_str!("../../../fuzz/corpus/parser_frontend/recovery.rttp");
const CHECKER_PIPELINE: &str = include_str!("../../../fuzz/corpus/checker/pipeline.rttp");
...
fn frontend_recovery_corpus_completes_with_bounded_diagnostics() {
    let source = SourceText::new("recovery.rttp", FRONTEND_RECOVERY);
...
fn complete_checker_corpus_preserves_all_or_nothing_conversion() {
    let source = SourceText::new("pipeline.rttp", CHECKER_PIPELINE);
