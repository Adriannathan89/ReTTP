# UTest 0.1.1 Release Plan

## Objective

Publish the first MVP release from `main` as tag `v0.1.1`, with native
...
3. every workspace package reports version `0.1.1`;
...
```text
commit documentation and workflow -> push main -> tag v0.1.1 -> push tag
...
or reused. Version `0.1.1` replaces the installer with a pinned Cargo install.
